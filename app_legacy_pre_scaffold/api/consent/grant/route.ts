...
export async function POST(request: NextRequest) {
  ...
  if (auth.profile.role === "patient") {
    ...
    if (!ownPatient) {
      return NextResponse.json({ error: "Patient can only grant consent for own profile" }, { status: 403 });
    }
  }

  const { data, error } = await auth.supabase
    .from("consents")
    .upsert({ ...parsed.data, granted: true }, { onConflict: "patient_id,organization_id,access_type" })
